...
QUnit.test("Check counter one", function(assert) {
  assert.equal(AnalyticsCounters.limits.limit0, 50000, 'Limit is ' + AnalyticsCounters.limits.limit0);
});

QUnit.test("Check second counter", function(assert) {
  assert.equal(AnalyticsCounters.limits.limit1, 1032, 'Limit is ' + AnalyticsCounters.limits.limit1);
});

QUnit.test("Check third counter", function(assert) {
  assert.equal(AnalyticsCounters.limits.limit2, 5400, 'Limit is ' + AnalyticsCounters.limits.limit2);
});

QUnit.test("Check last counter ", function(assert) {
  assert.equal(AnalyticsCounters.limits.limit3, 10300, 'Limit is ' + AnalyticsCounters.limits.limit3);
});
